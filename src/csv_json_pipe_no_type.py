...
class Pipeline(object):
    def __init__(self, csv_filepath, sequence_length):
        ...
        self.csv_data = {}
        self.csv_data['train'], self.csv_data['test'] = train_test_split(self.csv_lines, test_size = 0.2, shuffle = True, random_state = 42)
    ...
    def create_x_data(self):
        ...
        train_sentences = [record[3] for record in self.csv_data['train']]
        test_sentences = [record[3] for record in self.csv_data['test']]
    ...
    def create_y_data(self):
        ...
        self.train_y = np.vstack([np.array([int(record[4]),int(record[5]),int(record[6]),int(record[7])]) for record in self.csv_data['train']])
        self.test_y = np.vstack([np.array([int(record[4]),int(record[5]),int(record[6]),int(record[7])]) for record in self.csv_data['test']])
